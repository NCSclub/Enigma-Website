    ...
    <section className="h-screen w-screen text-white">
      <div className="h-full w-full flex flex-col justify-center items-center gap-10">
        ...
        <div className="flex gap-9">
          <button onClick={() => handleButtonClick(1)} className="bg-[#15153F] h-20 w-44 rounded-lg font-text text-2xl font-bold hover:bg-blue-500 hover:shadow-2xl hover:shadow-blue-500">DAY1</button>
          <button onClick={() => handleButtonClick(2)} className="bg-[#15153F] h-20 w-44 rounded-lg font-text text-2xl font-bold hover:bg-blue-500 hover:shadow-2xl hover:shadow-blue-500">DAY2</button>
        </div>
        <div className="h-1/3 w-2/4 p-5 bg-[#15153F] rounded-lg flex justify-center items-center gap-10">
          {agenda[index].time.map((time, i) => (
            <AgendaItem key={i} time={time} title={agenda[index].title[i]} />
          ))}
        </div>
      </div>
    </section>
